...
export class AgentHistoryProvider {
  ...
  async manageHistory(
    ...
    const { messagesToKeep, messagesToTruncate } = this.splitHistoryForTruncation(history);
  ...
  private async getSummaryText(
    ...
    if (!this.config.isExperimentalAgentHistorySummarizationEnabled()) {
      debugLogger.log('AgentHistoryProvider: Summarization disabled, using fallback note.');
      return this.getFallbackSummaryText(messagesToTruncate);
    }

    try {
      const summary = await this.generateIntentSummary(messagesToTruncate, abortSignal);
