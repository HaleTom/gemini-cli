...
describe('AgentHistoryProvider', () => {
  ...
  beforeEach(() => {
    config = {
      ...
      isExperimentalAgentHistorySummarizationEnabled: vi.fn().mockReturnValue(false),
  ...
  it('should return history unchanged if truncation is disabled', async () => {
    vi.spyOn(config, 'isExperimentalAgentHistoryTruncationEnabled').mockReturnValue(false);
  ...
  it('should return history unchanged if length is under threshold', async () => {
    vi.spyOn(config, 'isExperimentalAgentHistoryTruncationEnabled').mockReturnValue(true);
  ...
  it('should truncate mechanically to RETAINED_MESSAGES without summarization when sum flag is off', async () => {
    vi.spyOn(config, 'isExperimentalAgentHistoryTruncationEnabled').mockReturnValue(true);
    vi.spyOn(config, 'isExperimentalAgentHistorySummarizationEnabled').mockReturnValue(false);
    ...
    expect(result[0].parts![0].text).toContain('System Note: Prior conversation history was truncated');
  });

  it('should call summarizer and prepend summary when summarization is enabled', async () => {
    vi.spyOn(config, 'isExperimentalAgentHistoryTruncationEnabled').mockReturnValue(true);
    vi.spyOn(config, 'isExperimentalAgentHistorySummarizationEnabled').mockReturnValue(true);
  ...
  it('should handle summarizer failures gracefully', async () => {
    ...
    expect(result.length).toBe(15);
    expect(result[0].role).toBe('user');
    expect(result[0].parts![0].text).toContain('System Note: Prior conversation history was truncated and summarization failed.');
  });
});
